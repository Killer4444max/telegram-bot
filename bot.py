...
def extract_line(caption: str, key: str) -> str | None:
    m = re.search(rf"(?im)^{key}\s*:\s*(.+)$", caption)
...
def parse_product_caption(caption: str) -> dict | None:
    ...
    bolim = extract_line(caption, "BOLIM") or extract_line(caption, "BO'LIM")
    name = extract_line(caption, "NOM")
    price = extract_line(caption, "NARX")
    code = extract_line(caption, "KOD")
    color = extract_line(caption, "RANG") or ""

    if not bolim or not name or not price or not code:
        return None

    raw_price = re.sub(r"[^\d]", "", price)
    if not raw_price.isdigit():
        return None

    category = clean_category(bolim)
...
def get_products_by_category(category: str) -> list[tuple[str, dict]]:
...
async def show_category(update: Update, context: ContextTypes.DEFAULT_TYPE, category: str):
    ...
    if not products_list:
        await update.message.reply_text(
            f"{title} bo‘limi uchun hali mahsulot joylanmagan.\n\n"
            "Post formati:\n"
            f"BOLIM: {category}\n"
            "NOM: Pijama\n"
            "NARX: 120000\n"
    ...
    for code, product in products_list:
        ...
        if len(photo_ids) == 1:
            ...
            for i, photo_id in enumerate(photo_ids[:10]):
                media.append(InputMediaPhoto(media=photo_id, caption=caption if i == 0 else None))
...
async def order_entry_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data["category"] = product.get("category", "")
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🛍 Mahsulotlar":
        ...
        await show_category(update, context, "pijama")

    elif text == "🥻 Pinuar":
        await show_category(update, context, "pinuar")

    elif text == "🌸 Parfumeriya":
        await show_category(update, context, "parfum")
